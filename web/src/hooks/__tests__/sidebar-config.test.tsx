...
vi.mock('@/hooks/use-console-mode', () => ({ useConsoleMode: () => 'developer' }))
...
function sidebarFor(admin?: object, user?: object, canConfigure = true) {
  ...
  useAuthStore.getState().auth.setBundle({
    access_token: 'test-session-token', token_type: 'Bearer', access_expires_at: 2000000000,
    session: { sid: 'test-session', current: true, login_method: 'password', ip: '', user_agent: '', created_at: 1, last_active_at: 1, expires_at: 2000000000 },
    user: { id: 1, username: 'alice', role: 1, permissions: { sidebar_settings: canConfigure }, sidebar_modules: user ? JSON.stringify(user) : '' },
  })
  function Wrapper(props: { children: ReactNode }) {
    return (
      <QueryClientProvider client={client}>
...
describe('audit log sidebar entry', () => {
  ...
  it('legacy configurations show a separate Audit Logs link immediately after Usage Logs', () => {
    ...
    const items =
      result.current.find((group) => group.id === 'development')?.items ?? []
