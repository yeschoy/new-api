/*
Copyright (C) 2023-2026 QuantumNous

This program is free software: you can redistribute it and/or modify
it under the terms of the GNU Affero General Public License as
published by the Free Software Foundation, either version 3 of the
License, or (at your option) any later version.

This program is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU Affero General Public License for more details.

You should have received a copy of the GNU Affero General Public License
along with this program. If not, see <https://www.gnu.org/licenses/>.

For commercial licensing, please contact support@quantumnous.com
*/
import { render, screen } from '@testing-library/react'
import { createInstance } from 'i18next'
import { I18nextProvider, initReactI18next } from 'react-i18next'
import { describe, expect, test, vi } from 'vitest'

vi.mock('@tanstack/react-router', () => ({
  Link: ({ to, children }: { to: string; children: React.ReactNode }) => (
    <a href={to}>{children}</a>
  ),
}))

vi.mock('@/lib/lobe-icon', () => ({
  getLobeIcon: () => <span data-testid='tool-icon' />,
}))

const { Hero } = await import('../hero')

const i18n = createInstance()
await i18n.use(initReactI18next).init({
  lng: 'en',
  resources: {
    en: {
      translation: {
        'First three minutes': 'First three minutes',
        'Connect AI to the app you already use.':
          'Connect AI to the app you already use.',
        'You do not need to know how servers work. Create a key, copy an address, paste a model name, then send 你好.':
          'You do not need to know how servers work. Create a key, copy an address, paste a model name, then send 你好.',
        'Get Started': 'Get Started',
        'Create a free account': 'Create a free account',
        'Usage guide': 'Usage guide',
        'Works with': 'Works with',
      },
    },
  },
})

describe('landing hero', () => {
  test('sends signed-in users to the start page, not the keys table', () => {
    render(
      <I18nextProvider i18n={i18n}>
        <Hero isAuthenticated />
      </I18nextProvider>
    )

    expect(
      screen.getByRole('heading', {
        name: 'Connect AI to the app you already use.',
      })
    ).toBeInTheDocument()
    expect(screen.getByRole('link', { name: /Get Started/ })).toHaveAttribute(
      'href',
      '/dashboard'
    )
    expect(screen.getByText('Cherry Studio')).toBeInTheDocument()
    expect(screen.getByText('Trae')).toBeInTheDocument()
  })
})
