  ...
  const completeOAuthBinding = async () => {
    ...
    const result = await security.run(async (signal) => {
      ...
      try {
        ...
        if (callback.bindReturnResult) {
          throw new AuthOperationError(t('OAuth authorization was cancelled.'))
        }
        const outcome = callback.handoffTicket
          ? await authResult<AccountSecurityResult>(
              api.post('/api/oauth/domain-bind-handoff',
                { ticket: callback.handoffTicket },
                { ...authRequestOptions, singleUseAuthorization: true, signal: exchange.signal }
              )
            )
          : await authResult<AccountSecurityResult>(
