/*
Copyright (C) 2023-2026 QuantumNous

This program is free software: you can redistribute it and/or modify
it under the terms of the GNU Affero General Public License as
published by the Free Software Foundation, either version 3 of the
License, or (at your option) any later version.

This program is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU Affero General Public License for more details.

You should have received a copy of the GNU Affero General Public License
along with this program. If not, see <https://www.gnu.org/licenses/>.

For commercial licensing, please contact support@quantumnous.com
*/
import { render, screen } from '@testing-library/react'
import type { ReactNode } from 'react'
import { describe, expect, it, vi } from 'vitest'

import { EasyOverviewDashboardView } from '@/features/dashboard/components/overview/easy-overview-dashboard'
import { PricingSidebar } from '@/features/pricing/components/pricing-sidebar'
import { FILTER_ALL, QUOTA_TYPES } from '@/features/pricing/constants'

vi.mock('@tanstack/react-router', () => ({
  Link: (props: { children: ReactNode; className?: string; to: string }) => (
    <a className={props.className} href={props.to}>
      {props.children}
    </a>
  ),
}))

vi.mock('@/lib/lobe-icon', () => ({
  getLobeIcon: () => null,
}))

describe('easy-mode manual recovery', () => {
  it('keeps manual entry and model/group pricing reachable', () => {
    render(
      <>
        <EasyOverviewDashboardView
          remainQuota={1_000_000}
          usedQuota={0}
          connectPanel={
            <section>
              <span>API address</span>
              <span>API key</span>
              <span>Model</span>
            </section>
          }
          savings={{
            officialCost: 0,
            siteCost: 0,
            savings: 0,
            comparableRequests: 0,
          }}
        />
        <PricingSidebar
          quotaTypeFilter={QUOTA_TYPES.ALL}
          endpointTypeFilter={FILTER_ALL}
          vendorFilter={FILTER_ALL}
          groupFilter={FILTER_ALL}
          tagFilter={FILTER_ALL}
          onQuotaTypeChange={vi.fn()}
          onEndpointTypeChange={vi.fn()}
          onVendorChange={vi.fn()}
          onGroupChange={vi.fn()}
          onTagChange={vi.fn()}
          vendors={[]}
          groups={['standard', 'priority']}
          groupRatios={{ standard: 0.5, priority: 1 }}
          tags={[]}
          models={[]}
          hasActiveFilters={false}
          onClearFilters={vi.fn()}
        />
      </>
    )

    expect(screen.getByText('API address')).toBeVisible()
    expect(screen.getByText('API key')).toBeVisible()
    expect(screen.getByText('Model')).toBeVisible()
    expect(screen.getByRole('link', { name: 'Model prices' })).toHaveAttribute(
      'href',
      '/pricing'
    )
    expect(screen.getByText('Groups')).toBeVisible()
    expect(screen.getByRole('button', { name: /standard/i })).toHaveTextContent(
      'x0.5'
    )
    expect(screen.getByRole('button', { name: /priority/i })).toHaveTextContent(
      'x1'
    )
  })
})
