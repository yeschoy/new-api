...
import { setTestAuthUser } from '@/test-utils/auth-bundle'
...
beforeEach(() => {
  ...
  setTestAuthUser({ id: 1, username: 'admin', role: ROLE.ADMIN })
...
describe('administrator update entry', () => {
  test.each([null, ROLE.USER])(
    ...
    (role) => {
      setTestAuthUser(role === null ? null : { id: 2, username: 'user', role })
      render(
        <>
          <SystemUpdateAction />
  ...
  test.each([ROLE.ADMIN, ROLE.SUPER_ADMIN])(
    ...
    async (role) => {
      setTestAuthUser({ id: 1, username: 'admin', role })
...
describe('version notification preferences', () => {
  ...
  test('keeps preferences across logout and isolates ignore and restore actions by administrator', async () => {
    ...
    act(() =>
      setTestAuthUser({ id: 2, username: 'other-admin', role: ROLE.ADMIN })
    )
    expect(hook.result.current.shouldNotify).toBe(true)
    act(() => hook.result.current.setIgnored(true))
    act(() => setTestAuthUser({ id: 1, username: 'admin', role: ROLE.ADMIN }))
