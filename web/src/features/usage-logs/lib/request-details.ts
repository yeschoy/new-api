...
export function getDynamicBillingDetails(
  ...
  for (const entry of summary.priceEntries) {
    ...
    if (entry.unit === 'request') {
      quantity = 1
    } else if (entry.unit === 'image') {
      quantity = other.image_count
    } else {
      quantity = usage[entry.key as keyof typeof usage]
    }
    if (
    ...
    if (entry.unit === 'image' && !Number.isSafeInteger(quantity)) return null
    if (quantity === 0) continue

    const costBeforeGroup =
      (quantity * entry.price * requestMultiplier) /
      (entry.unit ? 1 : 1_000_000)
