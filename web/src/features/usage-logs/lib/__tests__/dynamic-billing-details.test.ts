...
describe('dynamic request billing details', () => {
  ...
  it('uses the recorded token billing unit when fixed and token tiers share a label', () => {
    const summary = getTieredBillingSummary(
      makeOther({
        expr_b64: encode(
          'len < 1000 ? tier("base", fixed(0.01)) : tier("base", p * 2)'
        ),
        billing_unit: 'token',
        billing_usage: {
          p: 5,
        ...
        billing_cost_before_group: 0.00001,
      })
    )
  ...
  it('suppresses guessed prices when duplicate token tiers share a label', () => {
    expect(
      getTieredBillingSummary(
        makeOther({
          expr_b64: encode(
            'len < 1000 ? tier("base", p * 1) : tier("base", p * 2)'
          ),
          billing_unit: 'token',
        })
      )
  ...
  it('itemizes fixed request pricing without token-unit conversion', () => {
    const details = getDynamicBillingDetails(
      makeOther({
        expr_b64: encode('tier("request", fixed(0.01))'),
        ...
        billing_cost_before_group: 0.02,
      })
    )

    expect(details?.lineItems).toEqual([
      {
        key: 'fixed',
        labelKey: 'Per-call',
        quantity: 1,
        unitPrice: 0.01,
        unit: 'request',
        costBeforeGroup: 0.02,
      },
    ])
  })

  it('itemizes fixed image pricing using the settled image count', () => {
    const details = getDynamicBillingDetails(
      makeOther({
        expr_b64: encode('tier("image", fixed(0.04)) * image_count'),
        matched_tier: 'image',
        billing_unit: 'request',
        fixed_price: 0.04,
        image_count: 3,
        billing_cost_before_group: 0.12,
      })
    )
