...
function makeFixedImageLog(): UsageLog {
  const log = makeLog()
  const other = JSON.parse(log.other) as Record<string, unknown>
  other.expr_b64 = Buffer.from(
    'tier("image", fixed(0.04)) * image_count',
    'utf8'
  ).toString('base64')
  other.matched_tier = 'image'
  other.billing_unit = 'request'
  other.fixed_price = 0.04
  other.image_count = 3
  other.billing_cost_before_group = 0.12
  return { ...log, quota: 60_000, other: JSON.stringify(other) }
}
...
describe('dynamic request detail billing', () => {
  ...
  it('renders fixed image billing details with an image unit', async () => {
    const user = userEvent.setup()
    const client = await renderDetails(makeFixedImageLog())
    const trigger = screen.getByRole('button', { name: 'Billing Details' })

    await user.hover(trigger)
    const bill = await screen.findByRole('tooltip', { name: 'Billing Details' })
    expect(
      within(bill).getByText('Per image').closest('div')
    ).toHaveTextContent('3 × ¥0.04/image¥0.12')
    expect(
      screen.queryByText('Cache usage is included in the input price.')
    ).not.toBeInTheDocument()
    client.clear()
  })
})
