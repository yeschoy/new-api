...
import { isDomainBindReturnResult } from './domain-oauth-handoff'
...
export function openOAuthPopup(
  ...
  return new Promise((resolve, reject) => {
    ...
    const onMessage = (event: MessageEvent<unknown>) => {
      ...
      const domainMessage = event.data as {
        type?: string
        provider?: string
        ticket?: string
        result?: unknown
      ...
      if (
        options.intent === 'bind' &&
        domainMessage?.type === OAUTH_BIND_RETURN_MESSAGE &&
        domainMessage.provider === options.provider &&
        isDomainBindReturnResult(domainMessage.result)
      ) {
        fail(new AuthOperationError('OAuth failed'))
        return
      }
      if (
        options.intent === 'bind' &&
        domainMessage?.type === OAUTH_BIND_HANDOFF_MESSAGE &&
        domainMessage.provider === options.provider &&
        typeof domainMessage.ticket === 'string' &&
        domainMessage.ticket.length > 0
      ) {
        received = true
        stopCloseWatcher()
        clearTimeout(deadline)
