  ...
  return (
    <CatalogPageLayout showMainContainer={false}>
      <div className='relative'>
        <PageTransition className='relative mx-auto w-full max-w-[1280px] space-y-6 px-3 pt-10 pb-10 sm:px-6 sm:pt-14 sm:pb-12 xl:px-8'>
...
function RankingsError(props: { message: string; onRetry: () => void }) {
  const { t } = useTranslation()
  return (
    <div className='ed-paper px-6 py-12 text-center'>
      <h2 className='ed-display text-xl'>
        {t('Unable to load rankings')}
      </h2>
      <p className='text-muted-foreground mx-auto mt-2 max-w-md text-sm'>
        {props.message}
      </p>
      <Button variant='outline' className='mt-4 rounded-full' onClick={props.onRetry}>
        {t('Retry')}
      </Button>
    </div>
  )
