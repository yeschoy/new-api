...
func StartTwoFASetup(identity AuthIdentity, authorization *model.AuthFlowAuthorization) (*TwoFASetup, error) {
	...
	flowToken, err := model.CreateTwoFAEnrollment(identity.SessionIdentity(), authorization, key.Secret(), codes, expiresAt)
	if err != nil {
		return nil, err
	}
...
func FinishTwoFASetup(identity AuthIdentity, flowToken, code string) error {
	...
	return model.EnableTwoFAEnrollment(identity.SessionIdentity(), flowToken, code)
}
