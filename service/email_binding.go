...
func StartEmailBinding(identity AuthIdentity, authorization *model.AuthFlowAuthorization, email string) (*EmailBindingData, error) {
	...
	token, flow, err := model.CreateEmailBinding(identity.SessionIdentity(), state)
	if err != nil {
		return nil, err
	}
...
func ResendAccountEmailBinding(identity AuthIdentity, token string) (*EmailBindingData, error) {
	_, state, err := model.GetEmailBinding(identity.SessionIdentity(), token)
	if err != nil {
		return nil, err
	}
	...
	flow, state, err := model.ResendEmailBinding(identity.SessionIdentity(), token, codes.NewHash, codes.OldHash)
	if err != nil {
		return nil, err
	}
	if err := sendEmailBindingCodes(*state, codes); err != nil {
...
func FinishEmailBinding(identity AuthIdentity, token, newCode, oldCode string) (*model.EmailBindingState, error) {
	...
	return model.CompleteEmailBinding(identity.SessionIdentity(), token, state.Email, newCode, oldCode)
}
