...
- `billingexpr.TokenParams` serializes as `p`, `c`, `len`, `cr`, `cc`, `cc1h`, `img`, `img_cr`, `img_o`, `ai`, and `ao`.
...
- A token line item is `normalized quantity × declared unit price / 1_000_000 × matched request-rule multipliers`. A fixed request/image line uses quantity `1` or the recorded `image_count` and does not divide by one million.
...
- When multiple tiers share a label, use the recorded `billing_unit` and `fixed_price` to disambiguate them. If the remaining candidate is not unique, hide unit prices instead of guessing by branch order.
...
| Duplicate same-label tiers remain ambiguous after recorded-unit filtering | Hide unit prices and itemization |
| Fixed image count is missing, non-integer, or negative | Hide itemization |
...
- Good: `tier("image", fixed(0.04)) * image_count` with a recorded count of 3 itemizes `3 × $0.04/image` and reconciles to `$0.12` before the group multiplier.
...
- Frontend model tests: same-label unit disambiguation, ambiguous duplicate suppression, and fixed request/image quantities without token scaling.
- Component tests: all declared prices remain visible; mouse hover and keyboard focus expose verified line items with token/request/image units; old logs omit the trigger.
