...
func TestConfiguredCacheWritePriceIsRecordedWithoutCacheUsage(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if !tt.wantPresent {
				assert.NotContains(t, snapshot, "cache_creation_ratio")
				return
			}
			require.Contains(t, snapshot, "cache_creation_ratio")
			assert.Equal(t, tt.wantRatio, snapshot["cache_creation_ratio"])
			assert.NotContains(t, snapshot, "cache_creation_tokens")
		})
	}
}
...
func TestClaudeCacheWriteDurationPricesAreRecordedWithoutCacheUsage(t *testing.T) {
	...
	snapshot := other.Snapshot()
	require.Contains(t, snapshot, "cache_creation_ratio_5m")
	require.Contains(t, snapshot, "cache_creation_ratio_1h")
	assert.Equal(t, 1.25, snapshot["cache_creation_ratio_5m"])
	assert.Equal(t, float64(2), snapshot["cache_creation_ratio_1h"])
	assert.NotContains(t, snapshot, "cache_creation_tokens_5m")
	assert.NotContains(t, snapshot, "cache_creation_tokens_1h")
}
