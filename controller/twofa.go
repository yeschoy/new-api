...
func Disable2FA(c *gin.Context) {
	...
	if err := model.DisableTwoFAForSession(identity.SessionIdentity()); err != nil {
		writeSecurityOperationError(c, err)
		return
	}
...
func RegenerateBackupCodes(c *gin.Context) {
	...
	// 保存新的备用码并原子推进用户鉴权版本
	if err := model.ReplaceBackupCodesForSession(identity.SessionIdentity(), backupCodes); err != nil {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "保存备用码失败",
		})
