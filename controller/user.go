...
func recordLoginAudit(user *model.User, c *gin.Context) {
	recordLoginAuditWithMethod(user, loginMethodFromContext(c), c)
}

func recordLoginAuditByUserID(userID int, method string, c *gin.Context) {
	user, err := model.GetSelfUserById(userID)
	if err != nil {
		common.SysError("login audit user lookup failed: " + err.Error())
		return
	}
	recordLoginAuditWithMethod(user, method, c)
}

func recordLoginAuditWithMethod(user *model.User, method string, c *gin.Context) {
	ip := c.ClientIP()
...
func UpdateSelf(c *gin.Context) {
	...
	if user.Password != "" {
		...
		if err := model.ChangeUserPassword(identity.SessionIdentity(), &cleanUser, firstPassword); err != nil {
			writeSecurityOperationError(c, err)
			return
		}
		succeeded = true
...
func DeleteSelf(c *gin.Context) {
	...
	if err := model.DeleteUserForSession(identity.SessionIdentity()); err != nil {
