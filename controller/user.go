...
func recordLoginAudit(user *model.User, c *gin.Context) {
	recordLoginAuditWithMethod(user.Id, user.Role, user.Username, loginMethodFromContext(c), c)
}

func recordLoginAuditByUserID(userID int, method string, c *gin.Context) {
	user, err := model.GetUserById(userID, false)
	if err != nil {
		common.SysError(fmt.Sprintf("failed to load user %d for login audit: %v", userID, err))
		return
	}
	recordLoginAuditWithMethod(user.Id, user.Role, user.Username, method, c)
}

func recordLoginAuditWithMethod(userID, role int, username, method string, c *gin.Context) {
	ip := c.ClientIP()
	...
	model.RecordLoginLog(userID, role, username, content, ip, "login", params, extra, c)
}
