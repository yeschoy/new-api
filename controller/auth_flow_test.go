...
func TestGenerateOAuthCodeCarriesAffiliateInLoginFlow(t *testing.T) {
	...
	callback := httptest.NewRequest(http.MethodGet, "/api/oauth/auth-flow-test?state="+response.Data.FlowToken+"&code=test", nil)
	callbackRecorder := httptest.NewRecorder()
	callbackContext, _ := gin.CreateTestContext(callbackRecorder)
	callbackContext.Request = callback
	callbackContext.Params = gin.Params{{Key: "provider", Value: "auth-flow-test"}}
	HandleOAuth(callbackContext)

	assert.Equal(t, http.StatusForbidden, callbackRecorder.Code)
	assert.Zero(t, provider.exchangeCalls, "a callback from another browser must be rejected before token exchange")
	_, err = model.GetAuthFlow(response.Data.FlowToken, model.AuthFlowMatch{
		Purpose: model.AuthFlowPurposeOAuth, Provider: "auth-flow-test", Intent: model.AuthFlowIntentLogin,
	})
	assert.NoError(t, err, "a browser-binding failure must not consume the flow")
}
