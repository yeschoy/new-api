...
func TestPasskeyDomainPreviewConfirmationAndAudit(t *testing.T) {
	...
	assert.ErrorIs(t, model.RegisterPasskeyForSession(identity.SessionIdentity(), &model.PasskeyCredential{UserID: user.Id, RPID: &lower, CredentialID: "replacement", PublicKey: "key"}), system_setting.ErrPasskeyRPIDUnavailable)
}
...
func TestPasskeyRegistrationAndDomainRemovalSerialize(t *testing.T) {
	...
	go func() {
		registered <- model.RegisterPasskeyForSession(identity.SessionIdentity(), &model.PasskeyCredential{UserID: user.Id, RPID: &rpID, CredentialID: "concurrent", PublicKey: "key"})
	}()
	<-registering
